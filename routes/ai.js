...
router.post("/generate-quiz", requireHost, aiLimiter, async (req, res) => {
  try {
    ...
    const validated = questions.map(validateQuestion).filter(q => q.question && q.options.length === 4);
...
router.post("/regenerate-question", requireHost, aiLimiter, async (req, res) => {
  try {
    const { content, audience = "General Church", tone = "Conversational", existingQuestions = [] } = req.body;
    if (!content?.trim()) return res.status(400).json({ error: "Content is required." });
...
router.post("/generate-from-reference", requireHost, aiLimiter, async (req, res) => {
  try {
    ...
    if (!response.ok)
      return res.status(400).json({ error: "Could not fetch Bible verse. Check the reference." });
    ...
    res.json({ questions: validated, count: validated.length, referenceText: data.text, resolvedReference: data.reference });
  } catch (err) {
    console.error("AI verse generate error:", err.message);
    res.status(500).json({ error: "AI verse generation failed: " + err.message });
  }
});
