...
function setupSocketHandlers(io) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join-game', ({ pin, name }, callback) => {
      ...
      if (!safeName) return callback({ success: false, error: 'Invalid name.' });

      if ([...game.players.values()].find(p => p.name.toLowerCase() === safeName.toLowerCase())) {
      ...
      if (game.dbGameId) {
        prisma.player.upsert({
          ...
          update: {},
          create: { name: safeName, gameId: game.dbGameId },
    ...
    socket.on('rejoin-game', ({ pin, name }, callback) => {
      ...
      if (pending) {
        ...
        if (playerData) {
          ...
          game.players.set(socket.id, { id: socket.id, name: safeName, score: 0, streak: 0, answered: false, lastAnswer: null });
        }
      } else {
        const already = [...game.players.values()].find(p => p.name === safeName);
        if (already) {
