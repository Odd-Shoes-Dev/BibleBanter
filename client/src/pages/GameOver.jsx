export default function GameOver({ leaderboard, playerName, onPlayAgain, role, onViewReport, onContinue, continueInfo }) {
  ...
  const rankMsg = (rank) => {
    if (rank === 1) return { emoji: '🏆', msg: 'CHAMPION!', color: 'text-amber-400', bg: 'bg-amber-500/20 border-amber-500/40' };
    if (rank === 2) return { emoji: '🥈', msg: 'RUNNER-UP!', color: 'text-slate-300', bg: 'bg-slate-500/20 border-slate-500/40' };
    if (rank === 3) return { emoji: '🥉', msg: '3RD PLACE!', color: 'text-orange-400', bg: 'bg-orange-500/20 border-orange-500/40' };
    return { emoji: `#${rank}`, msg: `RANK ${rank}`, color: 'text-white/70', bg: 'bg-white/10 border-white/20' };
  };
  ...
  return (
    <div className="min-h-screen flex flex-col px-4 py-6 overflow-y-auto relative" style={{ background: '#0d0918' }}>
      ...
      <div className="w-full max-w-2xl mx-auto space-y-6 relative z-10">
        ...
        {/* Player rank card */}
        {myResult && playerName && (
          <div className={`rounded-3xl p-6 text-center border animate-slide-up ${myResult.bg}`}>
            <div className="text-5xl mb-2">{myResult.emoji}</div>
            <p className={`font-nunito text-2xl font-black ${myResult.color}`}>{myResult.msg}</p>
            <p className="text-white font-bold text-xl mt-1">{playerName}</p>
            <p className="text-white/60 mt-2">
              Final Score: <span className="text-white font-black font-nunito text-2xl">{myEntry?.score?.toLocaleString() || 0}</span>
            </p>
            {myEntry?.streak >= 3 && (
              <p className="text-orange-400 text-sm mt-1">🔥 Best streak: {myEntry.streak}x</p>
            )}
          </div>
        )}
        ...
        {leaderboard.length > 0 && (
          <div className="bg-glass-dark rounded-3xl p-5 animate-slide-up" style={{ animationDelay: '0.2s' }}>
            ...
            <div className="space-y-2">
              {leaderboard.map((player, i) => {
                ...
                return (
                  <div key={player.name}
                    className={`flex items-center gap-3 rounded-xl px-4 py-2.5 ${isMe
                    ...
                    <span className={`flex-1 font-semibold truncate text-sm ${isMe ? 'text-amber-300' : 'text-white/80'}`}>
                      {player.name}
                      {isMe && <span className="text-amber-400/50 text-xs ml-1">(you)</span>}
                    </span>
                    {player.streak >= 3 && <span className="text-orange-400 text-xs">🔥{player.streak}</span>}
                    <span className={`font-nunito font-black text-base ${isMe ? 'text-amber-300' : 'text-white/70'}`}>
