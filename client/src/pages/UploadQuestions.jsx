...
export default function UploadQuestions({
  ...
  const handleImport = async () => {
    ...
    try {
      if (token && setName.trim()) {
        ...
        if (!data.savedSetId)
          throw new Error("Set was not saved — please log in again and retry.");
        setSuccess(
          `✅ "${setName.trim()}" saved with ${data.count} questions!`,
        );
        setTimeout(() => {
          onImported(data.savedSetId);
          onClose();
        }, 1400);
        return;
      }
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-2xl rounded-3xl overflow-hidden animate-bounce-in"
        style={{
          background: "#151626",
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          {!preview && !error && (
            <div
              className="rounded-xl px-4 py-3 text-xs text-white/40 leading-relaxed"
              style={{
                background: "rgba(255,255,255,0.04)",
                border: "1px solid rgba(255,255,255,0.07)",
              }}
            >
              <p className="font-bold text-white/50 mb-1">
                PDF / Word format (one block per question):
              </p>
              ...
              <p className="mt-1.5 text-amber-400/70 text-xs">
                📖 <strong>Scripture is required</strong> — it shows to players
                after the answer is revealed.
              </p>
              <p className="mt-2">
                For CSV, download the template ↑ to see the required columns.
              </p>
            </div>
          )}
          ...
          {error && (
            <div
              className="rounded-xl px-4 py-3 text-sm font-semibold animate-bounce-in"
              style={{
          ...
          {success && (
            <div
              className="rounded-xl px-4 py-3 text-sm font-semibold animate-bounce-in text-center"
              style={{
                ...
                {preview.slice(0, 20).map((q, i) => (
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <span className="text-white/30 text-xs font-bold w-5 flex-shrink-0 mt-0.5">
                        {i + 1}.
                      </span>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1">
                          {q.options.filter(Boolean).map((opt, j) => (
                            <span
                              key={j}
                              className={`text-xs ${j === q.answer ? "font-black" : "text-white/40"}`}
                              style={{
                                color: j === q.answer ? "#4ade80" : undefined,
                              }}
                            >
                              {["A", "B", "C", "D"][j]}
                              {j === q.answer ? " ✓" : ""}: {opt}
        ...
        {preview && preview.length > 0 && (
          ...
          >
            ...
            >
              {importing
                ? "⏳ Importing..."
                : `⚔️ Use These ${preview.length} Questions`}
            </button>
          </div>
        )}
      </div>
